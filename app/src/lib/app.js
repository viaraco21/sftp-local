...
const atualizausuario = async (req, res) => {
  const usuarioDesejado = req.params.usuario;
  const { pass, groupId } = req.body; //atualizar a senha e o grupo.
  ...
  try {
    ...
    for (const linha of linhas) {
      const [usuario, pass, groupId] = linha.split(':');
      ...
      if (usuario === usuarioDesejado) {
        // Encontrou o usuário, atualize as informações
        novoConteudo += `${usuario}:${pass}:${groupId}\n`;
    ...
    //O conteúdo é escrito de forma síncrono, 
...
const incluiusuario = async (req, res) => {
  const { userId, pass, groupId } = req.body; // incluir um novo usuário com senha e grupo.
  ...
  try {
    ...
    const novaLinha = `${userId}:${pass}:${groupId}\n`;
